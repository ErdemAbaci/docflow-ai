...
- [x] **Hata yönetimi:** başarısız işleme → `status: "failed"` + `errorReason` Cosmos'a (`markFailed()`), hata yeniden fırlatılıyor ki Service Bus'ın retry/DLQ mekanizması (max delivery count 5, built-in DLQ) devrede kalsın.
- [x] **Secrets:** dış sır olan NVIDIA API key Key Vault'a taşındı; Container Apps'e **managed identity** (user-assigned) ile bağlandı — Cosmos/Storage/Service Bus/Document Intelligence için RBAC (`azurerm_role_assignment` + Cosmos'a özel `azurerm_cosmosdb_sql_role_assignment`), kod tarafında `DefaultAzureCredential` → **ADR-4**. Gerçek Azure'a karşı `e2e-test.ts` ile doğrulandı.
- [x] Worker iş mantığı unit testleri (vitest) — `messageHandler.ts`'e çıkarılan iş mantığı, Extractor/Cosmos/Blob/OCR mock'lanarak test edildi (skip/not-found/success/failure senaryoları).
