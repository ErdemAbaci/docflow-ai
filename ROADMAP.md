...
- [ ] 💰 **Idempotency (LLM çağrısından ÖNCE yapılmalı):** Service Bus at-least-once teslimat yapar — worker Cosmos'a yazıp mesajı `complete` etmeden çökerse mesaj tekrar gelir. AI devredeyken her tekrar teslimat **yeni bir LLM çağrısı = para**, üstelik `max_delivery_count: 5` ile 5 kata kadar. Çözüm: worker işe başlamadan kaydı okusun, `status !== "queued"` ise atlasın. Aynı zamanda mülakatta en çok sorulan queue konusu.
